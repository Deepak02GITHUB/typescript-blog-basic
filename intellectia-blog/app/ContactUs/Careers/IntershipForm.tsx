...
export default function InternshipForm({ onClose }: InternshipFormProps) {
  ...
  // ✅ Correct useEffect (called unconditionally)
  useEffect(() => {
    if (loading) document.body.style.overflow = "hidden";
    else document.body.style.overflow = "";
  }, [loading]);

  // If not on correct path, render nothing
  if (pathname !== "/ContactUs/Careers") return null;
  ...
  function handleChange(field: string, value: string | File) {
    ...
    setErrors((prev) => {
      const copy = { ...prev };
      if ((typeof value === 'string' && value.trim() !== '') || (field === 'selectedFile' && value instanceof File)) {
        delete copy[field];
      }
      return copy;
    });
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      if (file.size > maxSize) setError("File size exceeds the 5MB limit.");
      else if (!allowedExtensions.test(file.name)) setError("File must be PDF or DOCX.");
      else if (!fileNameRegex.test(file.name)) setError('Invalid file name. Rename as &quot;YourName_Resume.pdf&quot; or .docx.');
      else {
        setSelectedFile(file);
        setError("");
      }
  ...
  const handleReset = () => {
    setFirstName(''); setLastName(''); setEmail(''); setPhone(''); setCountryCode('+91');
    setCurrentYear(''); setGraduationStatus(''); setLocation(''); setUniversity('');
    setBatch(''); setPractise(''); setDescription(''); setSelectedFile(null);
  };
  ...
  const handleSubmit = async () => {
    ...
    if (!email.trim()) newErrors.email = "Email is required";
    if (!phone.trim()) newErrors.phone = "Phone is required";
    if (!graduationStatus.trim()) newErrors.graduationStatus = "Graduation status is required";
    if (!university.trim()) newErrors.university = "University name is required";
    if (!location.trim()) newErrors.location = "Location is required";
    if (!currentYear.trim()) newErrors.currentYear = "Current year is required";
    if (!batch.trim()) newErrors.batch = "Batch year is required";
    ...
    if (email && !/^[a-zA-Z0-9._%+-]+@gmail\.com$/.test(email)) newErrors.email = "Email must be @gmail.com";
    ...
    try {
      let uploadedFileData = selectedFile ? await uploadFile(selectedFile) : null;

      const formData = {
        firstName, lastName, email, phone, graduationStatus, university,
        location, currentYear, batch, practise, description,
        uploadedFile: uploadedFileData ? { id: uploadedFileData.id, url: uploadedFileData.url } : null,
      };

      const response = await fetch(`${process.env.NEXT_PUBLIC_STRAPI_API_URL}/api/intership-application/send`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      setErrorMessage(error.message || "Failed to send internship application.");
      setShowErrorPopup(true);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <AnimatePresence mode="wait">
      ...
      >
        <div className="inset-0 flex items-start justify-center font-dm-sans">
          <div className="bg-white w-full max-w-3xl rounded-md shadow-lg relative flex flex-col h-[80vh] overflow-y-auto">
            {/* Loading, Success, Error popups omitted for brevity */}
            <div className="px-6 md:px-8 py-6 flex-1 space-y-8 -ml-3">
              {/* Other input fields */}
              <div>
                <label className="block text-sm font-medium text-black mb-3 text-left ml-1">
                  Upload your Resume
                </label>
                <div
                  className={`w-full border-2 border-dashed rounded-md p-6 text-center text-sm relative ${
                    errors.selectedFile ? 'border-red-600 text-red-600' : 'border-gray-400 text-gray-500'
                  }`}
                >
                  {!selectedFile ? (
                    <>
                      <p className="mb-4 text-gray-500">
                        Please ensure your uploaded resume is named using the format: &quot;YourName_Resume.pdf&quot;
                      </p>
                    </>
                  ) : (
                    <div className="flex items-center justify-between bg-gray-100 px-4 py-2 rounded-md text-gray-800">
                      <span className="truncate">{selectedFile.name}</span>
                      <button onClick={handleRemoveFile} className="text-red-600 text-xs hover:underline">Remove</button>
                    </div>
                  )}
                </div>
              </div>
