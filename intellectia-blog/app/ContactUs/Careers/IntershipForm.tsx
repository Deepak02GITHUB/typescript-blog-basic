...
export default function InternshipForm({ onClose }: InternshipFormProps) {
  ...
  // ✅ Conditional render AFTER hooks (fixes react-hooks/rules-of-hooks)
  if (pathname !== "/ContactUs/Careers") return null;
  ...
  function handleChange(field: string, value: string | File) {
    switch (field) {
      ...
      case 'graduationStatus': setGraduationStatus(value as string); break;
      case 'university': setUniversity(value as string); break;
      case 'location': setLocation(value as string); break;
      case 'currentYear': setCurrentYear(value as string); break;
      case 'batch': setBatch(value as string); break;
      case 'practise': setPractise(value as string); break;
      case 'description': setDescription(value as string); break;
      case 'selectedFile': setSelectedFile(value as File); break;
    }
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > maxSize)
      ...
      setError("Invalid type. Only PDF or DOCX allowed.");
    else if (!fileNameRegex.test(file.name))
      setError('Rename file as &quot;YourName_Resume.pdf&quot; or &quot;.docx&quot;.');
    else {
      setSelectedFile(file);
      setError("");
    }
  ...
  const handleReset = () => {
    ...
    setCountryCode('+91'); setCurrentYear(''); setGraduationStatus('');
    setLocation(''); setUniversity(''); setBatch('');
    setPractise(''); setDescription(''); setSelectedFile(null);
  };
  ...
  async function uploadFile(file: File): Promise<{ id: number; url: string }> {
    ...
    const uploaded = data[0];
    return { id: uploaded.id, url: `${process.env.NEXT_PUBLIC_STRAPI_API_URL}${uploaded.url}` };
  }
  ...
  const handleSubmit = async () => {
    ...
    if (!graduationStatus) newErrors.graduationStatus = "Graduation status required";
    if (!university) newErrors.university = "University required";
    if (!location) newErrors.location = "Location required";
    if (!currentYear) newErrors.currentYear = "Current year required";
    if (!batch) newErrors.batch = "Batch required";
    if (!practise) newErrors.practise = "Practise required";
    if (!description) newErrors.description = "Description required";
    if (!selectedFile) newErrors.selectedFile = "Resume required";
    ...
    try {
      ...
      const formData = {
        firstName, lastName, email, phone, graduationStatus, university,
        location, currentYear, batch, practise, description,
        uploadedFile: uploaded ? { id: uploaded.id, url: uploaded.url } : null,
      };
      ...
      setShowSuccessPopup(true); handleReset();
    } catch (err: any) {
      console.error(err);
  ...
  return (
    <AnimatePresence mode="wait">
      <motion.div
        ...
        animate="visible"
        exit="exit"
        className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 font-dm-sans overflow-auto"
      >
        <div className="bg-white rounded-2xl shadow-2xl p-6 md:p-10 w-[95%] md:w-[750px] my-10">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-2xl font-semibold text-gray-800">Internship Application</h2>
            <button onClick={onClose} className="text-gray-500 hover:text-gray-700 text-xl">&times;</button>
          </div>

          <p className="mb-4 text-gray-500">
            Please ensure your uploaded resume is named using the format:
            &nbsp;<span className="font-medium">&quot;YourName_Resume.pdf&quot;</span>
          </p>

          <div className="grid md:grid-cols-2 gap-4">
            <input value={firstName} onChange={e => handleChange('firstName', e.target.value)} placeholder="First Name"
              className="border rounded-lg p-2 w-full" />
            <input value={lastName} onChange={e => handleChange('lastName', e.target.value)} placeholder="Last Name"
              className="border rounded-lg p-2 w-full" />
            <input type="email" value={email} onChange={e => handleChange('email', e.target.value)} placeholder="Email"
              className="border rounded-lg p-2 w-full" />
            <div className="flex gap-2">
              ...
              <input value={phone} onChange={e => handleChange('phone', e.target.value)} placeholder="Phone"
                className="border rounded-lg p-2 w-2/3" />
            </div>
            <input value={university} onChange={e => handleChange('university', e.target.value)} placeholder="University"
              className="border rounded-lg p-2 w-full" />
            <input value={location} onChange={e => handleChange('location', e.target.value)} placeholder="Location"
              className="border rounded-lg p-2 w-full" />
            <input value={currentYear} onChange={e => handleChange('currentYear', e.target.value)} placeholder="Current Year"
              className="border rounded-lg p-2 w-full" />
            <input value={batch} onChange={e => handleChange('batch', e.target.value)} placeholder="Batch"
              className="border rounded-lg p-2 w-full" />
            <input value={graduationStatus} onChange={e => handleChange('graduationStatus', e.target.value)}
              placeholder="Graduation Status" className="border rounded-lg p-2 w-full" />
            <input value={practise} onChange={e => handleChange('practise', e.target.value)} placeholder="Area of Expertise"
              className="border rounded-lg p-2 w-full" />
          </div>

          <textarea value={description} onChange={e => handleChange('description', e.target.value)}
            placeholder="Tell us about yourself" className="border rounded-lg p-2 w-full mt-4 h-28" />

          <div className="mt-4 flex flex-col gap-2">
            <label className="font-medium text-gray-700">Upload Resume</label>
            <input type="file" onChange={handleFileChange} className="border p-2 rounded-lg w-full" />
            {error && <p className="text-red-600 text-sm">{error}</p>}
            {selectedFile && (
              <div className="flex items-center justify-between bg-gray-100 p-2 rounded-md">
                <span className="text-sm">{selectedFile.name}</span>
                <button onClick={handleRemoveFile} className="text-red-500 hover:text-red-700 text-sm">Remove</button>
              </div>
            )}
          </div>

          <div className="mt-6 flex justify-end gap-4">
            <button onClick={handleReset} className="px-5 py-2 rounded-lg border hover:bg-gray-50">Reset</button>
            <button onClick={handleSubmit} className="px-5 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700 flex items-center gap-2">
              {loading && <Oval width={18} height={18} color="#fff" secondaryColor="#ccc" />}
              Submit
            </button>
          </div>
          ...
          {showSuccessPopup && (
            <div className="fixed inset-0 flex items-center justify-center bg-black/50">
              <div className="bg-white p-6 rounded-xl shadow-lg text-center">
                ...
                <p className="text-gray-600">Thank you for applying. We’ll get back soon.</p>
                <button onClick={() => setShowSuccessPopup(false)} className="mt-4 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
                  Close
                </button>
              </div>
            </div>
          ...
          {showErrorPopup && (
            <div className="fixed inset-0 flex items-center justify-center bg-black/50">
              <div className="bg-white p-6 rounded-xl shadow-lg text-center">
                ...
                <button onClick={() => setShowErrorPopup(false)} className="mt-4 px-4 py-2 bg-gray-200 rounded-lg hover:bg-gray-300">
                  Close
                </button>
              </div>
            </div>
